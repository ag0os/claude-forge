...
describe("listTaskFiles", () => {
	...
	test("returns only .md files", async () => {
		const tasksDir = join(testDir, "forge", "tasks");
		await Bun.write(join(tasksDir, "TASK-001 - Test.md"), "content");
		await Bun.write(join(tasksDir, "TASK-002 - Test.md"), "content");
		await Bun.write(join(tasksDir, "notes.txt"), "content");
		await Bun.write(join(tasksDir, "config.json"), "{}");
	...
	test("returns files sorted alphabetically", async () => {
		const tasksDir = join(testDir, "forge", "tasks");
		await Bun.write(join(tasksDir, "TASK-003 - Third.md"), "content");
		await Bun.write(join(tasksDir, "TASK-001 - First.md"), "content");
		await Bun.write(join(tasksDir, "TASK-002 - Second.md"), "content");
